...
func NewRollbackService(s core.Store, c ContainerClient) *RollbackService {
...
// cleanupOldContainers removes only FREE containers from the inactive version
...
func (rs *RollbackService) cleanupOldContainers(ctx context.Context, functionName, previousVersion, currentActiveFunctionID string) error {
	...
	// Find the inactive function entry that matches the previousVersion
	var inactiveFunctionID string
	for _, v := range versions {
		if v.Version == previousVersion && v.Status == "inactive" {
			inactiveFunctionID = v.ID
			break
		}
	}

	if inactiveFunctionID == "" {
		// No inactive version found - nothing to clean
		return nil
	}

	// Get containers for the inactive function version
	containers, err := rs.store.GetContainersByFunction(inactiveFunctionID)
	if err != nil {
